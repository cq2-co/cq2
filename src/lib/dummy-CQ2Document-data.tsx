export const DummyCQ2DocumentData = {
  ...
  version1: {
    ...
    content:
      '<h2 class="text-xl font-semibold">‎Abstract</h2><p>‎This PEP proposes introducing syntactic sugar <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=)</code> for the pattern where a named argument is the same as the name of the variable corresponding to its value <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code>.</p><h2 class="text-xl font-semibold">‎Motivation</h2><p>‎Keyword argument syntax can become needlessly repetitive and verbose.</p><p>‎Consider the following call:</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎my_function(\n  my_first_variable=my_first_variable,\n  my_second_variable=my_second_variable,\n  my_third_variable=my_third_variable,\n)</code></pre><p>‎The case of a keyword argument name matching the variable name of its value is prevalent among Python libraries. This verbosity and redundancy discourages use of named arguments and reduces readability by increasing visual noise.</p><h2 class="text-xl font-semibold">‎Rationale</h2><p>‎There are two ways to invoke a function with arguments: by position and by keyword. Keyword arguments confer many benefits by being explicit, thus increasing readability and minimising the risk of inadvertent transposition. On the flipside, positional arguments are often used simply to minimise verbosity and visual noise.</p><p>‎We contend that a simple syntactic sugar used to simplify this common pattern which would confer numerous benefits:</p><h3 class="text-lg font-semibold">‎Encourages use of named variables</h3><p>‎This syntax would encourage the use of named variables, thereby increasing readability and reducing bugs from argument transposition.</p><h3 class="text-lg font-semibold">‎Reduces verbosity</h3><p>‎By minimising visual noise and in some cases lines of code, we can increase readability.</p><h3 class="text-lg font-semibold">‎Encourages consistent variable names</h3><p>‎A common problem is that semantically identical variables have different names depending on their contexts. This syntax would encourage authors to use the same variable name when calling a function as the argument name, which would increase consistency of variable names used and hence also readability.</p><h3 class="text-lg font-semibold">‎Highlights arguments not following this pattern</h3><p>‎With the current syntax, function calls where many arguments are forwarded from the local context can make other argument values easy to miss due to the visual noise. For example:</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎add_middleware(\n    excluded_urls=excluded_urls,\n    server_request=server_request,\n    client_request=client_request,\n    client_response=client_response,\n    span_details=_get_span_details(),\n    tracer=tracer,\n    meter=meter,\n)</code></pre><p>‎With this syntax, the exceptional arguments become easier to identify:</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎add_middleware(\n    excluded_urls=,\n    server_request=,\n    client_request=,\n    client_response=,\n    span_details=_get_span_details(),\n    tracer=,\n    meter=,\n)</code></pre><h3 class="text-lg font-semibold">‎Applicability to dictionary construction</h3><p>‎This syntax can be applied to dictionary construction where a similar pattern frequently occurs (where dictionary keys are identical the names of the variables assigned as their values), <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎{"x": x, "y": y}</code> or <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎dict(x=x, y=y)</code>. With this feature, this can now also be trivially written as <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎dict(x=, y=)</code>. Whether to further support similar syntax in dictionary literals is an open question out of the scope of this PEP.</p><h2 class="text-xl font-semibold">‎Specification</h2><p>‎We propose to introduce syntactic sugar such that, if the value of a keyword argument is omitted from a function invocation, the argument’s value is inferred to be the variable matching that name at the invocation scope.</p><p>‎For example, the function invocation:</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎my_function(my_first_variable=, my_second_variable=, my_third_variable=)</code></pre><p>‎Will be interpreted exactly equivalently to following in existing syntax:</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎my_function(\n  my_first_variable=my_first_variable,\n  my_second_variable=my_second_variable,\n  my_third_variable=my_third_variable,\n)</code></pre><p>‎If no variable matches that name in the invocation scope, a <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎NameError</code> is raised in an identical manner as would be with the established expanded syntax.</p><p>‎This proposal only pertains to function invocations; function definitions are unaffected by the syntax change. All existing valid syntax is unchanged.</p><h2 class="text-xl font-semibold">‎Backwards Compatibility</h2><p>‎Only new syntax is added which was previously syntactically erroneous. No existing valid syntax is modified. As such, the changes proposed are fully backwards compatible.</p><h2 class="text-xl font-semibold">‎Security Implications</h2><p>‎There are no security implications for this change.</p><h2 class="text-xl font-semibold">‎How to Teach This</h2><p>‎Programmers may learn about this feature as an optional abbreviated syntax where keyword arguments are taught. The <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://docs.python.org/3/glossary.html#term-argument">Python Glossary</a> and <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://docs.python.org/3/tutorial/controlflow.html#keyword-arguments">Tutorial</a> may be updated accordingly.</p><h2 class="text-xl font-semibold">‎Prior Art</h2><p>‎Python already possesses a very similar feature in f-string interpolation where <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f\'{x=}\'</code> is effectively expanded to <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f\'x={x}\'</code> (see <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://github.com/python/cpython/issues/80998">related GitHub issue</a>).</p><p>‎Several modern languages provide similar features during function invocation, sometimes referred to as ‘punning’. For example:</p><ul class="list-disc ml-8"><li><p>‎In Ruby, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x:, y:)</code> is syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x: x, y: y)</code>. See the <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://www.ruby-lang.org/en/news/2021/12/25/ruby-3-1-0-released/#:~:text=Other%20Notable%20New%20Features">Ruby 3.1.0 release notes</a> (search for “keyword arguments”).</p></li><li><p>‎In ReasonML, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(~x, ~y)</code> is syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(~x=x, ~y=y)</code>. See the <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://reasonml.github.io/docs/en/function#function-application">ReasonML function documentation</a> (search for “punning”).</p></li><li><p>‎In SystemVerilog, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎(.mult, .mop1, .data);</code> is syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎(.mult(mult), .mop1(mop1),&nbsp; .data(data));</code>. See <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="http://www.sunburst-design.com/papers/CummingsDesignCon2005_SystemVerilog_ImplicitPorts.pdf">SystemVerilog Implicit Port Connections</a>.</p></li><li><p>‎In Jakt, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x, y)</code> is syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x: x, y: y)</code>. See <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://github.com/SerenityOS/jakt?tab=readme-ov-file#function-calls">The Jakt programming language</a>.</p></li></ul><p>‎Beyond function invocation specifically, more languages offer similar features:</p><ul class="list-disc ml-8"><li><p>‎In OCaml, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎let+ x in …</code> is syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎let+ x = x in …</code>. See <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://v2.ocaml.org/manual/bindingops.html#ss:letops-punning">OCaml Short notation for variable bindings (let-punning)</a>.</p></li><li><p>‎In JavaScript, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎{ x, y }</code> is syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎{x: x, y: y}</code>. See <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Object_initializer">JavaScript Object Initializer</a>.</p></li><li><p>‎In Rust, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎User { x, y }</code> is shorthand for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎User {x: x, y: y}</code>. See <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://doc.rust-lang.org/book/ch05-01-defining-structs.html#using-the-field-init-shorthand-when-variables-and-fields-have-the-same-name">Rust Using the Field Init Shorthand</a>.</p></li></ul><h2 class="text-xl font-semibold">‎Applicability</h2><p>‎We analysed popular Python libraries from the last few years using <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://gist.github.com/joshuabambrick/a850d0e0050129b9252c748fa06c48b2">this script</a> to compute:</p><ul class="list-disc ml-8"><li><p>‎The number of keyword arguments were of the form <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code> at invocation.</p></li><li><p>‎The percentage of keyword arguments which had the form <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code> at invocation.</p></li><li><p>‎The number of lines of code which could be saved by using this syntactic sugar to reduce the need for line wraps.</p></li></ul><p>‎The purpose of this exercise was to compute statistics about the prevalence of this pattern and should not be interpreted as a recommendation that the proposed syntactic sugar should be applied universally.</p><p>‎Based on <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline" href="https://peps.python.org/pep-0736/#applicability:~:text=Statistic,117">this table</a>, we note that the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code> keyword argument pattern is widespread, accounting for 10-20% of all keyword argument uses.</p><h2 class="text-xl font-semibold">‎Proposed Syntax</h2><p>‎While this feature has been proposed on numerous occasions with several different forms <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id13">[1]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id14">[2]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id15">[3]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id16">[4]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id17">[5]</a>, <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id18">[6]</a> we have opted to advocate for the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=)</code> form for the following reasons:</p><ul class="list-disc ml-8"><li><p>‎This feature has been proposed frequently over a ten year period with the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=)</code> or <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(=x)</code> being by far the most common syntax <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id13">[1]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id14">[2]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id18">[6]</a>. This is a strong indicator that it is the obvious notation.</p></li><li><p>‎The proposed syntax closely matches the f-string debug <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f\'{var=}\'</code> syntax (established Pythonic style) and serves an almost identical purpose.</p></li><li><p>‎The proposed syntax is exactly analogous to the Ruby keyword argument syntactic sugar. See the <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://www.ruby-lang.org/en/news/2021/12/25/ruby-3-1-0-released/#:~:text=Other%20Notable%20New%20Features">Ruby 3.1.0 release notes</a> (search for “keyword arguments”).</p></li><li><p>‎The syntax is easy to implement as it is simple syntactic sugar.</p></li><li><p>‎When compared to the prefix form (see <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference internal" href="https://peps.python.org/pep-0736/#rejected-ideas">Rejected Ideas</a>), this syntax communicates “here is a parameter, go find its argument” which is more appropriate given the semantics of named arguments.</p></li><li><p>‎<a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://discuss.python.org/t/syntactic-sugar-to-encourage-use-of-named-arguments/36217/130">A poll of Python developers</a> indicates that this is the most popular syntax among those proposed.</p></li></ul><h2 class="text-xl font-semibold">‎Rejected Ideas</h2><p>‎Many alternative syntaxes have been proposed however no syntax other than <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(=x)</code> or <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=)</code> has garnered significant support. We here enumerate some of the most popular proposed alternatives and why we ultimately reject them.</p><h3 class="text-lg font-semibold">‎<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(a, b, *, x)</code></h3><p>‎On a few occasions the idea has been floated to borrow the syntax from keyword-only function definitions.</p><p>‎In favour of this proposal:</p><ul class="list-disc ml-8"><li><p>‎This syntax is familiar from its use to require keyword-only arguments in function definitions.</p></li><li><p>‎<a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://discuss.python.org/t/syntactic-sugar-to-encourage-use-of-named-arguments/36217/130">A poll of Python developers</a> indicates that this is the second most popular syntax among those proposed.</p></li></ul><p>‎However, we object that:</p><ul class="list-disc ml-8"><li><p>‎For any given argument, it is less clear from local context whether it is positional or named. The <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎*</code> could easily be missed in a long argument list and named arguments may be read as positional or vice versa.</p></li><li><p>‎It is unclear whether keyword arguments for which the value was not elided may follow the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎*</code>. If so, then their relative position will be inconsistent but if not, then an arbitrary grouping is enforced between different types of keyword arguments and reordering would be necessary if only one name was changed.</p></li><li><p>‎The use of <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎*</code> in function calls is established and this proposal would introduce a new effect which could cause confusion. For example, <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(a, *x, y)</code> would mean something different than <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(a, *, x, y)</code>.</p></li></ul><h3 class="text-lg font-semibold">‎<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(=x)</code></h3><p>‎In favour of this form:</p><ul class="list-disc ml-8"><li><p>‎The prefix operator is more similar to the established <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎*args</code> and <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎**kwargs</code> syntax for function calls.</p></li><li><p>‎It draws more attention to itself when arguments are arranged vertically. In particular, if the arguments are of different lengths it is harder to find the equal sign at the end. Moreover, since Python is read left to right, the use of this feature is clearer to the reader earlier on.</p></li></ul><p>‎On the contrary:</p><ul class="list-disc ml-8"><li><p>‎While the prefix version is visually louder, in practice, there is no need for this feature to shout its presence any more than a typical named argument. By the time we read to the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎=</code> it is clear that the value is filled in automatically just as the value is clear in the typical keyword argument case.</p></li><li><p>‎Semantically, this form communicates ‘here is a value, fill in the parameter’ which is not what we want to convey.</p></li><li><p>‎It is less similar to f-string syntax.</p></li><li><p>‎It is less obvious that arbitrary expressions are invalid, e.g. <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(=a + b)</code>.</p></li></ul><h3 class="text-lg font-semibold">‎<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(%x)</code> or <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(:x)</code> or <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(.x)</code></h3><p>‎Several flavours of this syntax have been proposed with the prefix form substituting another character for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎=</code>. However, no such form has gained traction and the choice of symbol seems arbitrary compared to <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎=</code>. Additionally, there is less precedent in terms of existing language features (such as f-string) or other languages (such as Ruby).</p><h2 class="text-xl font-semibold">‎Objections</h2><p>‎There are only a few hard objections to the introduction of this syntactic sugar. Most of those not in favour of this feature are in the camp of ‘I wouldn’t use it’. However, over the extensive conversations about this feature, the following objections were the most common:</p><h3 class="text-lg font-semibold">‎The syntax is ugly</h3><p>‎This objection is by far the most common. On the contrary, we argue that:</p><ul class="list-disc ml-8"><li><p>‎This objection is subjective and many community members disagree.</p></li><li><p>‎A nearly-identical syntax is already established for f-strings.</p></li><li><p>‎Programmers will, as ever, adjust over time.</p></li></ul><h3 class="text-lg font-semibold">‎The feature is confusing</h3><p>‎We argue that:</p><ul class="list-disc ml-8"><li><p>‎Introducing new features typically has this impact temporarily.</p></li><li><p>‎The syntax is very similar to the established <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f\'{x=}\'</code> syntax.</p></li><li><p>‎The feature and syntax are familiar from other popular modern languages.</p></li><li><p>‎The expansion of <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x=</code> to <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x=x</code> is in fact a trivial feature and inherently significantly less complex than <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎*arg</code> and <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎**kwarg</code> expansion.</p></li><li><p>‎This particular syntactic form has been independently proposed on numerous occasions, indicating that it is the most obvious <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id13">[1]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id14">[2]</a> <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline footnote-reference brackets" href="https://peps.python.org/pep-0736/#id18">[6]</a>.</p></li></ul><h3 class="text-lg font-semibold">‎The feature is not explicit</h3><p>‎We recognise that, in an obvious sense, the argument value is ‘implicit’ in this proposed syntax. However, we do not think that this is what the Zen of Python is aiming to discourage.</p><p>‎In the sense that we take the Zen to be referring to, keyword arguments (for example) are more explicit than positional arguments where the argument name is omitted and impossible to tell from the local context. Conversely, the syntactic sugar for integers <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x += 1</code> is not more implicit than <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x = x + 1</code> in this sense, even though the variable is omitted from the right hand side, because it is immediately obvious from the local context what it is.</p><p>‎The syntax proposed in this PEP is much more closely analogous to the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x += 1</code> example (although simpler since we do not propose to introduce a new operation). Moreover, the introduction of this syntactic sugar should encourage the use of keyword arguments over positional ones, making typical Python codebases more explicit in general.</p><h3 class="text-lg font-semibold">‎The feature adds another way of doing things</h3><p>‎The same argument can be made against all syntax changes. This is a simple syntactic sugar, much as <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x += 1</code> is sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x = x + 1</code> when <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x</code> is an integer. This isn’t tantamount to a ‘new way’ of passing arguments but a more readable notation for the same way.</p><h3 class="text-lg font-semibold">‎Renaming the variable in the calling context will break the code</h3><p>‎A <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎NameError</code> would make the mistake clear in most cases. There may be confusion if a variable from a broader scope has the same name as the original variable, so no <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎NameError</code> would be raised. However, this issue can also occur with keyword arguments using the current syntax (arguably, this syntactic sugar could make it harder to spot). Moreover, having variables with the same name in different scopes is broadly considered bad practice and discouraged by linters.</p><p>‎Code editors could highlight the issue based on static analysis - <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=)</code> is exactly equivalent to writing <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code>. If <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x</code> does not exist, modern editors have no problem highlighting the issue.</p><h3 class="text-lg font-semibold">‎This syntax increases coupling</h3><p>‎We recognise that, as ever, all syntax has the potential for misuse and so should be applied judiciously to improve codebases. In this case, if a parameter and its value have the same semantics in both contexts, that may suggest that using this new syntax is appropriate and will help ameliorate the risk of unintentional desynchronisation which harms readability.</p><p>‎However, if the two variables have different semantics, that may suggest that this feature should not be used to encourage consistency or even that they should be renamed.</p><h2 class="text-xl font-semibold">‎Recommendations for using this syntax</h2><p>‎As with any other language feature, the programmer should exercise their own judgement about whether it is prudent to use it in any given context. We do not recommend enforcing a rule to use the feature in all cases where it may be applicable.</p><p>‎As described <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://peps.python.org/pep-0736/Thissyntaxincreasescoupling">above</a>, we propose that a reasonable rule of thumb would be to use this in cases where a parameter and its argument have the same semantics in order to reduce unintentional desynchronisation without causing inappropriate coupling.</p><h2 class="text-xl font-semibold">‎Impact on editing</h2><h3 class="text-lg font-semibold">‎Using a plain text editor</h3><p>‎Editing with a plain text editor should generally be unaffected.</p><p>‎When renaming a variable using a ‘Find-Replace’ method, where this syntax is used the developer will come across the function argument at invocation (as they would if this syntax was not used). At that point, they can as usual decide whether to update the argument as well or expand to the full <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code> syntax.</p><p>‎As with the current syntax, a ‘Find-Replace All’ method would fail since the keyword argument would not exist at function definition, in the vast majority of cases.</p><p>‎If the developer leaves the argument name unchanged and forgets to update its value, a <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎NameError</code> will typically be raised as described <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://peps.python.org/pep-0736/Renamingthevariableinthecallingcontextwillbreakthecode">above</a>.</p><h3 class="text-lg font-semibold">‎Proposals for IDEs</h3><p>‎In response to community feedback, we include some suggestions regarding how IDEs could handle this syntax. However, we of course defer to the domain experts developing IDEs to use their own discretion.</p><p>‎Most considerations are made simple by recognising that <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=)</code> is just syntactic sugar for <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code> and should be treated the same as at present.</p><p>‎Highlighting NameErrors</p><p>‎IDEs typically offer a feature to highlight code that may cause a <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎NameError</code>. We recommend that this syntax be treated similarly to the expanded form <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(x=x)</code> to identify and highlight cases where the elided value variable may not exist. What visual cue may be used to highlight these cases may be the same or different from that which would be used with the current syntax, depending on the IDE.</p><p>‎Jump to definition</p><p>‎There are a few possible ways that a ‘jump to definition’ feature could be implemented depending on the caret/cursor position.</p><p>‎One option is to:</p><ul class="list-disc ml-8"><li><p>‎Jump to the argument in the function definition if the caret/cursor is on the argument</p></li><li><p>‎Jump to the definition of the elided variable if the caret/cursor is on the character following the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎=</code> in our proposed syntax.</p></li></ul><p>‎Another, potentially complementary, option would be to expand the syntax visually on mouseover and enable a <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎Ctrl+Click</code> (or <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎Cmd+Click</code>) to the definition of the variable.</p><p>‎Rename symbol</p><p>‎There are a few ways that IDEs may wish to support a ‘Rename symbol’ feature for this syntax. For example, if the argument is being renamed, the IDE may:</p><ul class="list-disc ml-8"><li><p>‎Also rename the variable used as its value in each calling context where this syntax is used</p></li><li><p>‎Expand to use the full syntax to pass the variable used as its value</p></li><li><p>‎Prompt the developer to select between the two above options</p></li></ul><p>‎The last option here seems most preferable in order to reduce unintentional desynchronisation of names while highlighting the user to the changes.</p><h2 class="text-xl font-semibold">‎Reference Implementation</h2><p>‎<a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline reference external" href="https://github.com/Hels15/cpython/tree/last-build">A proposed implementation</a> for cpython has been provided by @Hels15.</p>',
    highlights: [
      {
        highlight_id: 0,
        start: "/p[7]/text()[1]",
    ...
    comments: [
      ...
      },
      {
        comment_id: 3,
        thread_id: 0,
        user_name: "Steve Dower",
        content:
          '<p>‎This is my first time seeing the proposal, and in general it’s an idea that I like (I implemented a similar concept in my own language a decade ago). I have two concerns:</p><ol class="list-decimal ml-8"><li><p>‎The <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x=, y=,</code> syntax looks incomplete. I’d prefer to have <em>something</em> after the equals, like <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x=*, y=*</code> or even <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x=..., y=...</code> (though I do appreciate that <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎x=</code> matches the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f"{x=}" --&gt; f"x={x}"</code> transform)</p></li><li><p>‎It makes editing code, and the implications of those edits, harder. This in on my mind because I watched a livestreamer today spend ages figuring out why her <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎{ authToken }</code> object in JavaScript worked, but <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎{ authToken2 }</code> did not. As much as I like the sugar, I like not-confusing-new-users even more.</p></li></ol>',
      ...
      },
      {
        comment_id: 6,
        thread_id: 0,
        user_name: "Antoine Pitrou",
        content:
          '<p>‎In general I’m +1 on the idea. I find that repeating named arguments is a pattern that happens often when you have public APIs with similar signatures that defer to common utilities. And sometimes these functions have <em>a lot</em> of arguments.</p><p>‎To the readability argument, I would add that vertical space is very important to me (much of my time is spent reading and browsing through code). That is the reason I oppose the use of Black wherever I can, because Black consumes vertical space like crazy and that makes reviewing code much harder. Besides the obvious advantage of less typing, being able to reduce vertical space through tighter packing of argument lists is a welcome improvement.</p><p>‎I’m not fond of the proposed syntax but I agree there doesn’t seem to be a better alternative on the table. I would be ok with any of the following, or slight variations thereupon:</p><ul class="list-disc ml-8"><li><p>‎<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(arg_name=)</code></p></li><li><p>‎<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(=arg_name)</code></p></li><li><p>‎<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎f(arg_name=&lt;)</code> (the sigil meaning “look to the left for the argument’s value”)</p></li></ul><p>‎I also remember that the <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎@</code> syntax for decorators was widely loathed when it was standardized, and years later everyone came to find it entirely ok.</p>',
    ...
    threads: [
        ...
        comments: [
          {
            ...
            user_name: "Paul Moore",
            content:
              '<p>‎This is the only benefit of the new syntax that I’d support. And while I do find having to type <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎dict(a=a, b=b)</code> annoying, it mostly only comes up at the REPL, or in throwaway code.</p>',
        ...
        comments: [
          {
            ...
            user_name: "Joshua Bambrick",
            content:
              "<p>‎Yes, this feature should be used only where this effect is desirable. I will extend the PEP to emphasise this consideration as in the thread description.</p>",
        ...
        comments: [
          {
            ...
            user_name: "Paul Moore",
            content:
              '<p>‎You didn’t address the point that this tightly couples variable names in the caller and parameter names in the callee. Whether that’s a technical coupling (if I change the name of the local variable, I have to change the syntax used, not just rename the variable in the call as well) or “social” coupling (people are encouraged to use over-generic names in their code to match the arguments in APIs they use), it’s not really been addressed. There’s also the broader coupling, in the sense that it’s not possible to use this feature effectively for two APIs that don’t use the same argument names for something (for example, one API uses the name <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎colour</code> for an argument, and another API uses <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎color</code>).</p>',
      ...
      },
      {
        ...
        comments: [
          ...
          },
          {
            ...
            user_name: "Joshua Bambrick",
            content:
              "<p>‎@pf_moore, I highly discourage formatters from applying this syntax globally by default for the agreed reasons. I’d be happy to add a suggestion in the PEP to think carefully before doing this.</p>",
          ...
          },
          {
            ...
            user_name: "Eric V. Smith",
            content:
              '<p>‎@pf_moore, Ah, got it. <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎black</code>, at least, controls this with “magic trailing commas”.</p><p>‎But back on subject: I don’t think the PEP should concern itself with what formatters might do.</p>',
          ...
          },
          {
            ...
            user_name: "Paul Moore",
            content:
              "<p>‎@ericvsmith, Agreed. But the PEP suggests (in the “applicability” section) that reducing line count by wrapping arguments is a benefit of the syntax. That’s just as much about what formatters might do (in terms of taking a stance on the “best” format). So let’s remove that as well</p>",
        ...
        comments: [
          {
            ...
            user_name: "Chris Angelico",
            content:
              "<p>‎Maybe, but at the same time, there are a huge number of places where they WILL use the same name, simply because it is the single most obvious name for something. When the concept is the same, the name will frequently be the same.</p>",
      ...
      },
      {
        ...
        comments: [
          ...
          },
          {
            ...
            for_new_thread_created_parent_comment_id: 1,
            for_new_thread_created_quote:
              "But you can criticise the PEP for ignoring that pote...",
            _id: {
        ...
        comments: [
          {
            ...
            user_name: "Joshua Bambrick",
            content:
              "<p>‎Haha, I think you need to take that one up with Noah Webster!</p>",
          ...
          },
          {
            ...
            user_name: "Paul Moore",
            content:
              '<p>‎Happy to do so. As someone born and living in England, I can confirm he’s wrong and <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎colour</code> is the correct spelling. Will you raise PRs on all the libraries that use <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎color</code>, or should I?</p>',
          ...
          },
          {
            ...
            user_name: "Chris Angelico",
            content:
              "<p>‎Oh, you go ahead. They’ll fit in just fine alongside all the other drive-by PRs these projects get : )</p>",
      ...
      },
      {
        ...
        quote_by: "Paul Moore",
        quote:
          "But you can criticise the PEP for ignoring that potential limit on its applicability.",
        comments: [
          {
            comment_id: 0,
            thread_id: 14,
          ...
          },
          {
            ...
            user_name: "Cornelius Krupp",
            content:
              '<p>‎Oh, actually: Has it been suggested to add this shortcut syntax also to match patterns? That does seem like a decent usecase. That is probably the context where <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎name=name</code> has been the most annoying to me.</p>',
        ...
        comments: [
          {
            ...
            user_name: "Cornelius Krupp",
            content:
              "<p>‎I did not make this claim. But realizing a problem and suggesting something that <em>might</em> be a solution does not solve a problem.</p>",
        ...
        comments: [
          {
            ...
            user_name: "Cornelius Krupp",
            content:
              "<p>‎That was my knee jerk reaction as well, but I got used to it. It might not be perfect, but none of the other proposals have looked better.</p>",
        ...
        comments: [
          {
            ...
            user_name: "Cornelius Krupp",
            content:
              '<p>‎Not an option because <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎...</code> is already a valid expression.</p>',
      ...
      },
      {
        ...
        comments: [
          ...
          },
          {
            comment_id: 1,
            thread_id: 19,
            user_name: "James Webber",
            content:
              "<p>‎Those mean two different things in a function signature–is that a big problem?</p><p>‎I think omitting the standard whitespace in those examples makes them needlessly confusing.</p>",
          ...
          },
          {
            ...
            user_name: "James Webber",
            content:
              "<p>‎I don’t think the function signature is a big problem either, but I was asking because it seems like it should be equivalently-okay in the function call.</p>",
          ...
          },
          {
            ...
            user_name: "Chris Angelico",
            content:
              "<p>‎They mean different, but very much related, things. Whereas the proposal to do this at the call site would have them mean quite different things.</p>",
      ...
      },
      {
        ...
        comments: [
          ...
          },
          {
            comment_id: 1,
            thread_id: 21,
            user_name: "Jacob Nilsson",
            content:
              '<p>‎While keyword arguments probably would be nice in C++, all competent IDEs/language servers allow the option for inlay hints which essentially removes the need for them to help with readability</p><p>‎ Quick note on inlay hints for those unaware</p><p>‎Inlay hints for function calls is where the editor adds the name of the parameter in front of the given argument so</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎test(variable_a, variable_b)</code></pre><p>‎becomes, with the things within <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎|</code>pipes<code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎|</code> added by the editor, for example</p><pre class="bg-[#F9F9F9] text-neutral-700 p-4 rounded-lg text-sm"><code>‎test(|a:| variable_a, |b:| variable_b)</code></pre><p>‎In fact, it might be possible (but I’m not sure how likely it is) that people who use tens of positional arguments in function calls are using inlay hints, in which case the problem is solved for them. Doesn’t help much when reviewing code on Github perhaps, but it helps avoiding mistakes.</p><p>‎The more I think about this the more I’m thinking that this is a tooling problem. (Even though I don’t think repeated tokens in a function call is a problem at all.)</p>',
        ...
        comments: [
          {
            ...
            user_name: "Cornelius Krupp",
            content:
              "<p>‎I actually already had run this script and wrote modifications of it. Out of those function calls 65% only use 0 or 1 arguments. Are you going to argue that adding keyword arguments there is any kind of improvement?</p><p>‎(I can’t actually replicate the exact number of 75000 calls locally because I don’t have a CPython clone right now, I am getting 55k. But I am not going to argue that either number is non-representative.)</p><p>‎Just because a call don’t use kwargs doesn’t mean that using them there would make sense.</p>",
          ...
          },
          {
            ...
            user_name: "Chris Angelico",
            content:
              "<p>‎Ah, that’s actually something I hadn’t thought to check. Hmm. I’m not sure where the threshold of readability would be. What would you consider the point at which the number of arguments starts to make keywording them beneficial - and would that point change if the syntax were less verbose?</p>",
          ...
          },
          {
            ...
            user_name: "Jacob Nilsson",
            content:
              "<p>‎In any case, 0 argument functions definitely shouldn’t count, and I think most people would agree 1 argument functions should count too (unless the function name very bad but that’s another issue).</p>",
          ...
          },
          {
            ...
            user_name: "Jacob Nilsson",
            content:
              "<p>‎As to where the limit of readability lies, I think it’s highly dependent on the context. I don’t think most people (well, developers anyway) would struggle to figure out what the arguments to create_vector3(x, y, z) mean, nor their order, but other three-variable functions may be harder to reason about. A large part of this, I think, is familiarity with the code i question.</p><p>‎I’m strongly against this proposal, I don’t think the repetition is a problem, and of you think writing extra characters is the problem then I suggest creating a tool that autocompletes keyword arguments for you. Maybe make it an extension to a language server?</p><p>‎ A remark kinda OT</p><p>‎Just as a side note, I do appreciate that using keyword arguments is better. We had a problem at $work where two C++ libraries that used quaternions used different calling conventions, one used w, x, y, z and the other x, y, z, w. However, had I rendered inlay hints with clangd this wouldn’t have been an issue, so that’s an anecdote in favor of these kinds problems being solved with editor support.</p>",
          ...
          },
          {
            ...
            user_name: "Cornelius Krupp",
            content:
              '<p>‎@chrisangelico, 0-arg (~15%) for sure shouldn’t count.</p><p>‎1-arg almost always shouldn’t count, but sometimes you have functions that take many keyword arguments and you are only overwriting one of them.</p><p>‎But assuming postional-or-keyword, I think 3-arg is about upper limit (thinking of to pygame APIs for example: <code class="bg-[#F9F9F9] text-neutral-700 p-0.5">‎pg.draw.line(surface, color, start, end, width)</code> is potentially not readable without keyword arguments).</p><p>‎But it also depends on the function definition, which the script can’t really analyze with. (and I am not even sure if I could define a good general pattern or statistic to look for)</p>',
          ...
          },
          {
            ...
            user_name: "Paul Moore",
            content:
              "<p>‎I have to say I’m becoming confused as to the point of all this. Are you trying to suggest that all of those calls that pass all 5+ arguments positionally are going to be improved by using keyword arguments? How could you possibly make a broad generalisation like that?</p>",
          ...
          },
          {
            ...
            user_name: "Chris Angelico",
            content:
              "<p>‎No, I’m not, but I’m trying to get an idea of the extent to which keyword arguments are currently being used, in response to an earlier claim that keyword arguments don’t need any encouragement since they’re already perfectly well encouraged. Welcome to long threads and long tangents.</p>",
      ...
      },
      {
        ...
        comments: [
          ...
          },
          {
            ...
            user_name: "Steve Dower",
            content:
              "<p>‎IDLE does not support it, and realistically that’s the only one we could possibly use to justify a language design decision.</p><p>‎I don’t think we should let any IDE features influence the language. It should be readable in B&amp;W plain text printed on a piece of paper, and then it will only be <em>more</em> readable elsewhere.</p><p>‎The only exception I make here is that it should be easy to search for it in a search engine. “What does equals sign in a function call mean in Python” seems like a reasonable thing to expect, so it already meets that for me.</p>",
          ...
          },
          {
            ...
            user_name: "Antoine Pitrou",
            content:
              '<p>‎@jacobnilson, So how does this work for code reviews in the Github UI, for example? The argument that IDEs obsolete attempts at making programming languages more ergonomic has never held water.</p><p>‎Even if your favorite text editor supports language servers (not everybody uses VSCode), language servers do not always work properly. pylance <a target="_blank" rel="noopener noreferrer nofollow" class="text-[#797874] underline" href="https://github.com/microsoft/pylance-release/issues/674"><u>does not seem </u></a>to support Cython, for example, and it probably does not support extensions generated using other tools such as nanobind or PyO3.</p>',
          ...
          },
          {
            ...
            user_name: "Steve Dower",
            content:
              "<p>‎I think with type hinting becoming so big, nobody is ever going to try and introspect native modules anymore. Their developers will need to start producing type stub files to go with them. So it’s not about “not working properly” and more about a fundamental premise (“everything is fully typed”) that leads to this result.</p><p>‎A syntactic change in pure Python code would obviously be supported.</p>",
      ...
      },
      {
        ...
        comments: [
          ...
          },
          {
            comment_id: 1,
            thread_id: 20,
            user_name: "Gregory P. Smith",
            content:
              "<p>‎I would not analyze the Python Standard Library if looking to figure out what any sort of modern common coding practices are. The standard library as a whole is a pretty “horrible” body of very old code that we do not like to make changes to on purpose. <em>(it wakes the scary Rabbit)</em></p><p>‎I’d probably try to select a diverse subset of modern projects that haven’t been around for more than a decade, if even five years, including application code and <em>perhaps</em> include some recently shared Jupyter or Colab notebooks.</p>",
          ...
          },
          {
            ...
            user_name: "Chris Angelico",
            content:
              "<p>‎That’s perfectly reasonable. My script is available if anyone wants to run it on their own codebases, and I’m happy to take recommendations for corpuses of code to analyze.</p>",
          ...
          },
          {
            ...
            thread_id: 20,
            user_name: "Antoine Pitrou",
            content: "<p>‎Can you give a link to your script again?</p>",
